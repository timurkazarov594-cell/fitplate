...
router.post("/nutrition/analyze", requireAuth, async (req: AuthRequest, res) => {
  ...
  // Determine access: free first analysis OR paid credits
  const hasFreeAnalysis = !user.freeAnalysisUsed;
  const hasPaidCredits = user.photoCredits > 0;

  if (!hasFreeAnalysis && !hasPaidCredits) {
    req.log.info({ userId: user.id }, "No credits and free analysis already used");
  ...
  try {
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        { role: "system", content: SYSTEM_PROMPT },
        {
          role: "user",
          content: [
            { type: "text", text: "Проверь изображение на запрещённый контент, затем если это еда — верни JSON с КБЖУ на русском языке." },
    ...
    const errorCode =
      typeof jsonValue === "object" && jsonValue !== null && "error" in jsonValue
        ? (jsonValue as { error?: unknown }).error
        : null;

    // Forbidden content — do NOT charge any credit
    if (errorCode === "FORBIDDEN_CONTENT") {
      req.log.warn({ userId: user.id }, "Forbidden content detected, no credit charged");
      res.status(422).json({ error: "Это изображение нельзя обработать." });
      return;
    }

    // Not food — do NOT charge any credit
    if (errorCode === "NO_FOOD_DETECTED") {
      req.log.info({ userId: user.id }, "No food detected, no credit charged");
    ...
    // Successful analysis — charge accordingly
    if (hasFreeAnalysis) {
      // Mark free analysis as used; do NOT deduct credits
      await db.update(usersTable).set({ freeAnalysisUsed: true }).where(eq(usersTable.id, user.id));
      req.log.info({ userId: user.id }, "Free analysis used");
    } else {
