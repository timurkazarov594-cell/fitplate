  ...
  return (
    <div className="relative flex items-center gap-3 bg-background border-b border-primary/20 px-4 py-2.5 text-sm text-foreground">
      ...
      <p className="flex-1 min-w-0 leading-snug">
        {platform === "ios-safari" && (
          <>
            Чтобы добавить сайт на экран «Домой», нажмите кнопку «Поделиться» <ShareIcon /> внизу экрана, затем «На экран «Домой»»
          </>
        )}
        {platform === "android-chrome" && deferredPrompt && (
          <>Установите приложение, чтобы добавить его на главный экран и открывать в один тап</>
        )}
        {platform === "android-chrome" && !deferredPrompt && (
          <>
            Чтобы добавить сайт на главный экран, нажмите на меню <MenuDotsIcon /> в браузере, затем «Добавить на главный экран»
          </>
        )}
      </p>
